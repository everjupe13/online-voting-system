...
onMounted(() => {
  ...
  if (window.matchMedia && window.matchMedia('(prefers-color-scheme: dark)').matches) {
    bodyRef.value.classList.remove('light')
    bodyRef.value.classList.add('dark')
  } else {
    bodyRef.value.classList.remove('dark')
    bodyRef.value.classList.add('light')
  }
    ...
    <div class="app-main-aside__top-row">
      <div class="app-main-aside__control-panel">
        <button @click.prevent="changeTheme" class="app-main-aside__control-panel__item">
          <svg class="app-main-aside__contol-panel__icon" width="483" height="511" viewBox="0 0 483 511"
        ...
        <button v-if="store.getters['auth/isAuth']" class="logout-button" @click.prevent="logout">Logout</button>
      </div>
